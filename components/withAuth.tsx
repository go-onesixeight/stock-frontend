...
const withAuth = (WrappedComponent: React.FC) => (props: any) => {
  ...
  if (isClient()) {
    ...
    if (route !== KEY_ROUTE.LOGIN && route !== KEY_ROUTE.REGISTER) {
      if (!isAuthenticated) {
        router.push(KEY_ROUTE.LOGIN);
        return null;
      } else if (route === "/") {
        router.push(KEY_ROUTE.STOCK); // default page after login when call root path
        return null;
      }
    } else {
      if (isAuthenticated) {
        router.push(KEY_ROUTE.STOCK); // default page after login
        return null;
      }
    }
